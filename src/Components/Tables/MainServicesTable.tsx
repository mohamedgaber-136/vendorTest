...
import { ServiceSearchField } from '../SearchField/ServiceSearchField';
...
export function MainServicesTable({ data }: MainServicesTableProps) {
  ...
  return (
    <>
      <ServiceSearchField initialData={data.data} setData={setFilteredData} />
          ...
          <TableBody>
            {filteredData.map((row, index) => (
              <TableRow key={`row-${index}`} className="even:bg-gray-100">
                ...
                <TableCell className="font-medium text-right py-5 gap-3" style={{ minWidth: "150px" }}>
                  {row.service.name}
                </TableCell>
                <TableCell className="font-medium text-right py-5" style={{ minWidth: "150px" }}>
                  <button
