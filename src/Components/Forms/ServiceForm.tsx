...
export const ServiceForm: React.FC<ServiceFormProps> = ({ data, type }) => {
  ...
  const goversOption = govers ? govers.data.map((governorate: Goverdata) => ({ value: governorate.id, label: governorate.name_ar })) : [];
  ...
  const onSubmit = async (values: InitialValues, { setSubmitting }: FormikHelpers<InitialValues>) => {
    try {
      if (type === "post") {
        ...
        const response = await addItem({ endpoint: `service-vendors/${data?.service_id}?_method=PUT`, newItem: values }).unwrap();
  ...
  const validationSchema = Yup.object().shape({
    ...
    cover: Yup.array().min(data?.cover?.length ? 0 : 4, "يجب أن يحتوي الغلاف على 4 صور على الأقل").required("الغلاف مطلوب"),
  });
      ...
      {(formik) =>
        loading ? (
          ...
          <Form className="flex flex-col gap-3">
            ...
            <SelectField item={{ placeHolder: "نوع الخدمة", name: "service_id", options: serviceOptions }} setCityId={undefined} />
            <SelectField item={{ placeHolder: "المحافظة", name: "governorate_id", options: goversOption }} setCityId={setCityId} />
            <SelectField item={{ placeHolder: "المدينة", name: "city_id", options: Cities?.data ?? [] }} setCityId={undefined} />
