...
export const StoryForm = ({ data }: { data: Partial<InitialValues> }) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    files: Yup.array().min(1, "At least 1 images are required").required("Images are required"),
    ...
    <Formik initialValues={initialValues}
      ...
      {(formik) => (
        ...
        <Form className="flex flex-col gap-3">
          ...
          <FileField type="post_images" formik={formik} endpoint="files" item={{ placeHolder: "اضافة صورة ", name: "files" }} />
          <Button type="submit" className="w-full sm:w-full bg-primaryColor text-white hover:white">
            اضافه قصه
            {isLoading && <div className="loader"></div>}
          </Button>
          {isSuccess && <div className="text-green-500 text-center">Item added successfully!</div>}
